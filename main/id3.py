...
class ID3:
	def __init__(self, pathToData,pathToNames):
		...
		self.data = []    #tap du lieu
		self.classes = []
		self.numAttributes = -1  #so luong thuoc tinh (6)
		self.attrValues = {}
		self.attributes = []	#cac thuoc tinh
		self.tree = None
		self.order = []

	# lay data: data, numAttributes, attributes
	def fetchData(self):
	...
	def recursiveGenerateTree(self, curData, curAttributes):
		...
		if len(curData) == 0:
			#Fail
			return Node(True, "Fail", None,None)
		elif allSame is not False:
			#return a node with that class
			return Node(True, allSame, None,None)
		elif len(curAttributes) == 0:
			#return a node with the majority class
			majClass = self.getMajClass(curData)
			return Node(True, majClass, None,None)
		else:
			(best,best_threshold,splitted,best_order) = self.splitAttribute(curData, curAttributes)         #####them best order
			remainingAttributes = curAttributes[:]
			remainingAttributes.remove(best)
			node = Node(False, best, best_threshold,best_order)                                             #####them best order
	...
	def splitAttribute(self, curData, curAttributes):
		...
		return (best_attribute,best_threshold,splitted,best_order)
...
class Node:
	def __init__(self,isLeaf, label, threshold, order):
		self.label = label
